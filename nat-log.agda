...
data log-result (x : ℕ)(b : ℕ) : Set where
  pos-power : (e : ℕ) → (s : ℕ) → b pow e + s ≡ x → e =ℕ 0 ≡ ff → log-result x b
...
log x b p1 p2 | (suc q) , r , u1 , u2 | no-power u rewrite sym u1 = pos-power 1 (b * q + r) lem refl
  where lem : b * 1 + (b * q + r) ≡ b + q * b + r
        lem rewrite *1{b} | *comm b q = +assoc b (q * b) r
log x b p1 p2 | (suc q) , r , u1 , u2 | pos-power e s u v rewrite sym u1 = 
  pos-power (suc e) (b * s + r) lem refl
